(function (MW) {
  ...
  const Actions = Object.freeze({
    REGISTER:'register', LOGIN:'login', LOGOUT:'logout', FORGOT_PASSWORD:'forgot_password', UPDATE_ROLE:'update_role', DELETE_USER:'delete_user',
    SAVE_SETTINGS:'save_settings', ADD_RECORD:'add_record', ADD_RECORDS_BATCH:'add_records_batch', DELETE_TRIP:'delete_trip', UPDATE_RECORD:'update_record', DELETE_RECORD:'delete_record',
  ...
  const EntityTypes = Object.freeze({FACILITY:'facility', TREATMENT_UNIT:'treatment_unit', HEALTH_ADMIN:'health_admin', DIRECTORATE:'directorate'});
  ...
  const ActionRoles = Object.freeze({
    ...
    add_record:[Roles.FACILITY_ENTRY,Roles.TREATMENT_ENTRY,Roles.SUPERVISOR,Roles.ADMIN], add_records_batch:[Roles.FACILITY_ENTRY,Roles.TREATMENT_ENTRY,Roles.SUPERVISOR,Roles.ADMIN],
    ...
    update_record:[Roles.SUPERVISOR,Roles.ADMIN], delete_record:[Roles.ADMIN], delete_trip:[Roles.ADMIN], save_settings:[Roles.ADMIN], get_users:[Roles.ADMIN], update_role:[Roles.ADMIN], delete_user:[Roles.ADMIN], get_audit_log:[Roles.ADMIN]
  });
  ...
  MW.Contracts=Object.freeze({version:'1.16',Actions,Roles,RoleList,AuthenticatedRoles,EntrySources,EntityTypes,ReconciliationStatus,ActionRoles,ErrorCodes,Limits,rolesFor,isPublic,canRole});
})(window.MedWaste);
