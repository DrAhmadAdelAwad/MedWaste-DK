(function(MW){
  ...
  function saveRegistrationCache(data){Storage.setJson(Storage.KEYS.registrationOptions,data||{facilities:[],healthAdmins:[],treatmentUnits:[],directorates:[]});Storage.setText(Storage.KEYS.registrationOptionsFetchedAt,Date.now());return data;}
  async function registrationOptions(options={}){const maxAgeMs=Number(options.maxAgeMs)||21600000,local=registrationFromCache();if(!options.force&&local&&registrationIsFresh(maxAgeMs)&&(local.facilities?.length||local.healthAdmins?.length||local.directorates?.length))return local;try{const r=await Api.post(Contracts.Actions.GET_REGISTRATION_OPTIONS);const data=r.data||{facilities:[],healthAdmins:[],treatmentUnits:[],directorates:[]};return saveRegistrationCache(data);}catch(error){if(local)return local;throw error;}}
