(function(MW){
  ...
  async function listBundle(options={}){const local=cachedBundle();if(!options.force&&isFresh())return local;try{const r=await Api.read(Contracts.Actions.GET_USERS),bundle={users:(r.data||[]).map(UserMapper.fromApi),entities:r.entities||{facilities:[],healthAdmins:[],treatmentUnits:[],directorates:[]}};return saveBundle(bundle);}catch(e){if(local)return local;throw e;}}
