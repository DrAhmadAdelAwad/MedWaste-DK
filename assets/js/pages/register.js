(function(MW){
  ...
  function renderAssignments(){
    ...
    if(type==='مديرية الشئون الصحية بالدقهلية'){
      const dir=directorates[0]||null;wrap?.classList.add('hidden');sel.required=false;
      if(dir){sel.innerHTML='';addOption(sel,dir,type);sel.value=dir.entityId;}
      else{sel.innerHTML='<option value="">تعذر تحديد المديرية — أعد تحميل الصفحة</option>';}
      return;
    }
    wrap?.classList.remove('hidden');
    let list=[];
    if(type==='إدارات صحية'){list=healthAdmins;if(label)label.textContent='اختر الإدارة الصحية التابعة لك *';}
    else{list=facilities.filter(x=>!type||x.mainType===type);if(label)label.textContent='المنشأة التابعة لك *';}
  ...
  document.getElementById('registerForm')?.addEventListener('submit',async event=>{
    ...
    const entityId=select()?.value||'';if(!entityId){message.innerText=type==='إدارات صحية'?'اختر الإدارة الصحية التابعة لك.':'اختر المنشأة التابعة لك.';message.classList.remove('hidden');return;}
    button.innerText='جاري إنشاء الحساب...';button.disabled=true;message.classList.add('hidden');
    try{
      await Auth.register({fullName:document.getElementById('fullName').value,jobTitle:document.getElementById('jobTitle').value,workplace:document.getElementById('workplace').value,mobile:document.getElementById('mobile').value,email,password,entityId,facilityId:entityId,registrationKey:Auth.registrationKeyFor(email)});
